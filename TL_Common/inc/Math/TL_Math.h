...
namespace TL_Math
{
	...
	struct Quaternion : public XMFLOAT4
	{
		...
		Vector3 ToEulerDeg() const noexcept;

		// Static functions
		static Quaternion CreateFromAxisAngle(const Vector3& axis, float angle) noexcept;
		static Quaternion CreateFromAxisAngleDeg(const Vector3& axis, float angleDeg) noexcept;

		// Rotates about y-axis (yaw), then x-axis (pitch), then z-axis (roll)
		static Quaternion CreateFromYawPitchRoll(float yaw, float pitch, float roll) noexcept;
		static Quaternion CreateFromYawPitchRollDeg(float yawDeg, float pitchDeg, float rollDeg) noexcept;

		// Rotates about y-axis (angles.y), then x-axis (angles.x), then z-axis (angles.z)
		static Quaternion CreateFromYawPitchRoll(const Vector3& angles) noexcept;
		static Quaternion CreateFromYawPitchRollDeg(const Vector3& _euler) noexcept;
